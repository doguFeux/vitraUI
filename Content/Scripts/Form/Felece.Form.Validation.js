Felece.Validation = {
    ...
    Actions: {
        init: function () {
            ...
            $.validator.addMethod(
                ...
                function (currVal, element) {
                    ...
                    var rxDatePattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*?[!@#$%^&*+/(),.?"'_:{}|<>=-])[a-zA-Z-!@#$%^&*+/(),.?"':{}|_<>=\d]{6,}$/;
                    var dtArray = currVal.match(rxDatePattern);
                    ...
                    return true;
                }, "Şifre en az 6 karakterden oluşmalıdır ve en az 1 büyük harf, 1 küçük harf, 1 rakam, 1 özel karakter içermelidir."
            );
            ...
            jQuery.validator.addMethod('passwordMatch', function (value, element) {
                ...
                var password = value
                var confirmPassword = $("#" + element.getAttribute("data-rule-passwordMatch")).val()

                // Check for equality with the password inputs
                if (password != confirmPassword) {
                    return false;
                } else {
                    // Clear error feedback on related password element.
        ...
        run: function () {
            // ReCaptcha check.
            var recaptchaIsValid = Felece.Validation.Actions.reCaptchaCheck();

            // Form jQuery validation check.
            var isFormValid = $(Felece.Form.Props.formElem).valid();
        ...
        reCaptchaCheck: function () {
            var recaptchaElem = Felece.Form.Props.formElem.querySelector(".lc-recaptcha");
            var recaptchaIsValid = true;

            if (recaptchaElem) {
                Felece.Form.Props.reCaptchaElem = recaptchaElem;
                ...
                if (recaptchaResponse === "") {
                    ...
                    Felece.Form.UX.ReCaptcha.invalid();
                }
                else {
                    Felece.Form.UX.ReCaptcha.valid();
                }
            }
